...
int nextUnusedDirEntry(dirblock_t block){
	for (int i = 0; i < DIRENTRYCOUNT; i++){
		if (block.entrylist[i].unused){
			printf("Found a free dir entry at pos %d \n", i);
			return i ;
		}
	}
...
// used to find the next UNUSED block for mymkdir
int nextDirectoryBlock(){
	for (int pos = 5; pos < MAXBLOCKS; pos++){
		if (FAT[pos] == UNUSED){
			printf("Found unused block: %d\n",pos) ;
			return pos ;
		}
...
MyFILE * myfopen(const char *filename, const char * mode){	// from official C library function
	...
	if (pos != -1){
		...
		pos = nextUnusedBlock() ;
		printf("myfopen nextUnusedBlock: %d\n",pos );
...
void myfputc(int b, MyFILE *stream){		// from official C library function
	...
	if (strcmp(stream -> mode, "w") != 0){
		...
		if (stream -> pos >= BLOCKSIZE){
			// looks for a free fat entry
			printf("\nmyfputc > putting chars in buffer\n");
			i = nextUnusedBlock() ;			
			FAT[fatIndex] = i ;
			printf("myfputc fatIndex: %d\n",fatIndex);
			FAT[i] = ENDOFCHAIN ;
			printf("myfputc i: %d\n",i);
			copyFAT() ;
...
int mymkdir(char* path){
	...
	int fatIndex = rootDirIndex+1;

	// for each part of the path
	while ((token = strtok_r(rest, "/" , &rest))){

		// set the blockchain
		int i = nextDirectoryBlock();
		FAT[fatIndex] = i ;
		FAT[i] = ENDOFCHAIN ;
		...
		// operating on last block before ENDOFCHAIN
		diskblock_t block = virtualDisk[fatIndex] ;

		// clears the UNUSED block's entrylist, just in case
		clearBlockEntries(fatIndex, &block) ;
		int entry = nextUnusedDirEntry(block.dir) ;

		// assign directory name (token) to block
		strcpy(block.dir.entrylist[entry].name, token) ;	

		// setting directory attrs
		block.dir.nextEntry = 0 ;
		block.dir.isdir = 1 ;	
		block.dir.entrylist[entry].unused = FALSE ;
		block.dir.entrylist[entry].firstblock = i ;
		
		// write to disk
		copyFAT() ;
		writeblock(&block,fatIndex) ;
